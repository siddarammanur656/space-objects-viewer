(function(root) {
  ...
  // Bounding box calcs
  const header_height =
    document.getElementById("metadata").offsetHeight +
    document.getElementById("ticks").offsetHeight;
  ...
  const time_scale = d3
    ...
    .rangeRound([width, 0]);
  const size_scale = d3.scaleLog().domain([30, 17]).range([0.5, 4]);
  const hmag_scale = d3
    .scaleLinear()
      ...
      .then(function(rows, errors) {
        ...
        asteroids
          ...
          .attr("class", function(d) {
            ...
            if (
              new RegExp("(" + d.closeApproach.getFullYear() + ".*)").test(
                d.name,
              )
            ) {
              className += " new";
          ...
          .attr("rx", function(d) {
            return size_scale(d.h) - Math.random() * 0.3;
          })
          .attr("cy", function(d) {
          ...
          .attr("transform", function(d) {
            if (Math.random() * 2 > 1)
              return (
                "rotate(34, " +
                [
                  time_scale(d.closeApproach),
                  lunar_distance_scale(d.ldNominal * LUNAR_DISTANCE),
                ].join(",") +
                ")"
              );
          });

        const bigOnes = rows.filter(function(val) {
          return val.h < 21;
        });
        asteroids
          .selectAll("ruler-label")
          .data(bigOnes)
          .enter()
          .append("text")
          ...
          .attr(
            "y",
            (d) => lunar_distance_scale(d.ldNominal * LUNAR_DISTANCE) + 20,
          )
          .attr("foo", function(d) {
            drawLabelLine(
              asteroids,
  ...
  function setupControls() {
    document
      .querySelector("input[name=show-big]")
      .addEventListener("change", function(e) {
        document.body.classList.toggle("show-rings-big");
      });
    document
      .querySelector("input[name=show-huge]")
      .addEventListener("change", function(e) {
        document.body.classList.toggle("show-rings-huge");
      });
    document
      .querySelector("input[name=show-onlight]")
      .addEventListener("change", function(e) {
        document.body.classList.toggle("onlight");
      });
    document
      .querySelector("input[name=show-new]")
      .addEventListener("change", function(e) {
  ...
  function drawEarthAndMoon() {
    ...
    const moon = earthAndMoon
      .append("text")
      .attr("class", "moon")
      .text("🌖")
      .attr("x", width / 2 - 7.5)
      .attr("y", lunar_distance_scale(LUNAR_DISTANCE) + 5.5);

    const moonLabel = earthAndMoon
      .append("text")
      .attr("class", "ruler-label")
      .text("Moon")
      .attr("x", width / 2 + 83)
  ...
  function drawGuideLines(classname, months) {
    ...
    guides
      .selectAll("guide")
      .data([
        time_scale(d3.timeMonth.offset(date, -1 * months)),
        time_scale(d3.timeMonth.offset(date, months)),
      ])
      .enter()
      .append("line")
      .attr("x1", (d) => d)
  ...
  function drawLabelLine(container, elStart, elEnd, onRightSide) {
    container
      ...
      .attr("d", function() {
        ...
        const step1 =
          elStartBox.x +
          multiplyer * (elStartBox.width + 3) +
          "," +
          (elStartBox.y + elStartBox.height / 2);
        const step2 =
          elStartBox.x +
          (multiplyer * elStartBox.width + 25) +
          "," +
          (elStartBox.y + elStartBox.height / 2);
        const step3 =
          elEndBox.x +
      ...
      .text(function(d) {
        ...
        if (d === MAX_LDS) {
          return (
            d +
            " LDs, or about " +
            (LUNAR_DISTANCE * d).toLocaleString() +
            "km from Earth"
          );
        }
        return d + " LDs";
  ...
  function drawVoronoi(data) {
    ...
    const voronoiGroup = sky.append("g").attr("class", "voronoi");
    ...
    voronoiGroup
      ...
      .on("mouseenter", function(evt, { data }) {
        if (data.name === JWST) {
          ...
          popover
            .select("#approach")
            .text("This telescope is in L2, orbiting the sun, its");
          ...
          popover
            ...
            .text(
              approachPrefix +
              " " +
              data.closeApproach.toLocaleDateString() +
              ".",
            );
          popover
            .select("#minimum")
            .html(
              distancePrefix +
              "<strong>" +
              data.ldNominal.toFixed(1) +
              " LDs</strong>, and its",
            );
          popover
            .select("#size")
            .text(hmag_scale(data.h).toFixed(1) + " meters.");
